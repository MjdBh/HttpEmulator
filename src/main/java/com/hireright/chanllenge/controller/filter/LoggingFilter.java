package com.hireright.chanllenge.controller.filter;

import com.fasterxml.jackson.annotation.JsonInclude.Include;
import com.fasterxml.jackson.core.JsonProcessingException;
import com.fasterxml.jackson.databind.ObjectMapper;
import com.hireright.chanllenge.controller.filter.entity.LoggingRequest;
import com.hireright.chanllenge.controller.filter.entity.LoggingResponse;
import com.hireright.chanllenge.controller.filter.wrapper.LoggingHttpServletRequestWrapper;
import com.hireright.chanllenge.controller.filter.wrapper.LoggingHttpServletResponseWrapper;
...
public class LoggingFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain filterChain) throws IOException, ServletException {
		...
		if (disableMarker) {
			log.info(resolvedRequestPrefix + getRequestDescription(requestWrapper));
		} else {
			log.info(requestMarker, resolvedRequestPrefix + getRequestDescription(requestWrapper));
		}

		filterChain.doFilter(requestWrapper, responseWrapper);

		if (disableMarker) {
			log.info(resolvedResponsePrefix + getResponseDescription(responseWrapper));
		} else {
			log.info(responseMarker, resolvedResponsePrefix + getResponseDescription(responseWrapper));
		}
